...
function buildCashflow(input = {}) {
  ...
  const lookbackStart = receiptLookbackStart < operatingLookbackStart ? receiptLookbackStart : operatingLookbackStart;
  ...
  for (const row of weekly) {
    for (const key of ["despatchActual", "despatchForecast", "receiptsActual", "receiptsForecast", "otherInflows", "supplierPayments", "marketingSpend", "variableCosts", "fixedCosts", "otherOutflows"]) row[key] = money(row[key]);
    row.openingCash = money(runningCash);
    const inflows = row.receiptsActual + row.receiptsForecast + row.otherInflows;
    const outflows = row.supplierPayments + row.marketingSpend + row.variableCosts + row.fixedCosts + row.otherOutflows;
