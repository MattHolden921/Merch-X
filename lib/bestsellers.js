...
const TRADE_METRICS_VERSION = "pnl-demand-despatch-v2";
...
function addOrderLineMetric(metrics, item) {
  ...
  if (!orderedQuantity || !netQuantity) return false;
  ...
  const variant = current.variants.get(variantId) || { revenue: 0, grossSales: 0, units: 0 };
...
function combineTradingMetrics(periods = []) {
  const periodMetrics = {};
  const bridgeKeys = ["grossRevenue", "discounts", "netRevenue", "shippingRevenue", "tax", "returnFees"];
...
function calculateProductFinancials(metric, variants, options = {}) {
  ...
  return {
    ...
    costedRevenueExVat: reportedFinancials?.netSalesExVat ?? costedRevenue / divisor,
