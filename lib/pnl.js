...
const COST_TYPES = new Set(["fixed_monthly", "per_order", "per_item", "per_reversed_item", "pick_pack", "percent_revenue", "percent_revenue_plus_per_order"]);
const REVENUE_BASES = new Set(["order_demand", "order_intake", "pnl_revenue"]);
...
function publicRule(rule = {}) {
  ...
  const sourceData = rule.data && typeof rule.data === "object" ? rule.data : {};
  const requestedRevenueBasis = text(rule.revenueBasis || rule.revenue_basis || sourceData.revenueBasis);
  const revenueBasis = REVENUE_BASES.has(requestedRevenueBasis) ? requestedRevenueBasis : "order_demand";
  return {
    id: text(rule.id),
...
function costBaseForOverlap(base, rule, range) {
  ...
  return {
    ...
    demandRevenue: number(base.demandRevenue || base.despatchRevenue || base.netRevenue) * ratio,
    orderIntakeRevenue: number(
      base.demandRevenue || base.newShippingRevenue
        ? number(base.demandRevenue) + number(base.newShippingRevenue)
        : base.despatchRevenue || base.netRevenue
    ) * ratio,
    pnlRevenue: number(base.operatingRevenue || base.netRevenue) * ratio,
    orders: number(base.orders) * ratio,
    units: number(base.units) * ratio,
    returnedUnits: number(base.returnedUnits) * ratio
  };
}

function revenueBasisValue(base, rule) {
  if (rule.revenueBasis === "order_intake") return base.orderIntakeRevenue;
  if (rule.revenueBasis === "pnl_revenue") return base.pnlRevenue;
  return base.demandRevenue;
}

function revenueBasisLabel(rule) {
  if (rule.revenueBasis === "order_intake") return "order intake";
  if (rule.revenueBasis === "pnl_revenue") return "P&L revenue";
  return "order demand";
}
...
function calculateCostRule(ruleInput, baseInput, rangeInput) {
  ...
  if (rule.costType === "fixed_monthly") {
  ...
  } else if (rule.costType === "per_reversed_item") {
    amountApplied = rule.amount * overlapped.returnedUnits;
    orderDrivenAmount = amountApplied;
    formula = `${money(rule.amount)} per reversed item`;
    ...
    amountApplied = rule.rate * revenueBasisValue(overlapped, rule);
    revenueDrivenAmount = amountApplied;
    formula = `${money(rule.rate * 100)}% of ${revenueBasisLabel(rule)}`;
  } else if (rule.costType === "percent_revenue_plus_per_order") {
    revenueDrivenAmount = rule.rate * revenueBasisValue(overlapped, rule);
    orderDrivenAmount = rule.amount * overlapped.orders;
    amountApplied = revenueDrivenAmount + orderDrivenAmount;
    formula = `${money(rule.rate * 100)}% of ${revenueBasisLabel(rule)} + ${money(rule.amount)} per order`;
  }
  return {
    ...rule,
...
function marketingForecastModel(actualPnl = {}, driversInput = {}) {
  ...
  const blendedReturn = Math.max(0, number(
    ...
    baseSpend > 0 ? number(actualPnl.demandRevenue || actualPnl.despatchRevenue) / baseSpend : 0
  ));
...
function breakEvenMarketingReturn(actualsInput, costRules = [], driversInput = {}) {
  ...
  const targetDailyDemand = Math.max(0, number(
    driversInput.targetDailyDemand ?? driversInput.targetDailySales,
    scenarioDemandBasis(actuals) / days
  ));
  const baseDrivers = {
    ...driversInput,
    targetDailyDemand,
  ...
  const revenueStep = Math.max(100, Math.min(5000, Math.max(scenarioDemandBasis(actuals), targetDailyDemand * days) * 0.01));
  const baseActuals = scenarioActuals(actuals, baseDrivers);
  const bumpedActuals = scenarioActuals(actuals, {
    ...baseDrivers,
    targetDailyDemand: targetDailyDemand + (revenueStep / days)
  });
  const basePnl = buildPnl(baseActuals, costRules, []);
  const bumpedPnl = buildPnl(bumpedActuals, costRules, []);
  const demandDelta = number(bumpedPnl.demandRevenue) - number(basePnl.demandRevenue);
  const profitDelta = number(bumpedPnl.operatingProfit) - number(basePnl.operatingProfit);
  const contributionMargin = demandDelta > 0 ? profitDelta / demandDelta : 0;
  ...
  return {
    ...
    contributionPerDemand: Math.round(contributionMargin * 100) / 100,
    contributionPerDespatch: Math.round(contributionMargin * 100) / 100,
    revenueStep: money(demandDelta)
  };
}
...
function normalizeActuals(input = {}) {
  ...
  const aovBasisRevenue = money(input.aovBasisRevenue ?? input.orderDemandRevenue ?? despatchRevenue);
  const productReversalRevenue = money(input.productReversalRevenue ?? (input.returns ? -Math.abs(number(input.returns)) : 0));
  const productReversalRate = Math.abs(demandRevenue) > 0
    ? Math.abs(productReversalRevenue) / Math.abs(demandRevenue)
    : 0;
  const productReversalNetRevenue = money(input.productReversalNetRevenue);
  const shippingReversalRevenue = money(input.shippingReversalRevenue);
  const saleAdjustments = money(input.saleAdjustments);
  const refundAuditAvailable = Boolean(input.refundAuditAvailable);
  const refundAuditStatus = text(input.refundAuditStatus || (refundAuditAvailable ? "available" : "not_run"));
  const pendingRefundAmount = Math.max(0, money(input.pendingRefundAmount));
  const successfulRefundAmount = Math.max(0, money(input.successfulRefundAmount));
  const failedRefundAmount = Math.max(0, money(input.failedRefundAmount));
  const pendingRefundCount = Math.max(0, Math.round(number(input.pendingRefundCount)));
  const successfulRefundCount = Math.max(0, Math.round(number(input.successfulRefundCount)));
  const failedRefundCount = Math.max(0, Math.round(number(input.failedRefundCount)));
  const refundTransactionCount = Math.max(0, Math.round(number(input.refundTransactionCount)));
  const refundRecordCount = Math.max(0, Math.round(number(input.refundRecordCount)));
  const matchedPendingRefundAdjustment = money(input.matchedPendingRefundAdjustment ?? (
    refundAuditAvailable ? Math.min(Math.max(saleAdjustments, 0), pendingRefundAmount) : 0
  ));
  const residualSaleAdjustments = money(input.residualSaleAdjustments ?? (saleAdjustments - matchedPendingRefundAdjustment));
  const accruedSalesRevenue = money(input.accruedSalesRevenue ?? (despatchRevenue - matchedPendingRefundAdjustment));
  const accruedNetRevenue = money(input.accruedNetRevenue ?? (netRevenue - matchedPendingRefundAdjustment));
  const newShippingRevenue = money(input.newShippingRevenue);
  const otherSalesRevenue = money(input.otherSalesRevenue);
  const excludedRevenueActivity = money(returnFees + otherSalesRevenue);
  const revenueClassificationNote = Math.abs(excludedRevenueActivity) >= 0.01
    ? `${excludedRevenueActivity.toLocaleString("en-GB", { style: "currency", currency: "GBP" })} of Shopify return fees, duties or other sales activity is retained in the sales reconciliation but excluded from P&L revenue until its VAT and product-cost treatment is classified.`
    : "";
  const costedNetSales = money(input.costedNetSales ?? input.grossMarginRevenue);
  const uncostedNetSales = money(input.uncostedNetSales);
  const costCoverageDenominator = Math.abs(costedNetSales) + Math.abs(uncostedNetSales);
  const costCoverage = costCoverageDenominator > 0 ? Math.abs(costedNetSales) / costCoverageDenominator : null;
  const adjustmentRate = Math.abs(residualSaleAdjustments) / Math.max(Math.abs(accruedSalesRevenue), 1);
  const marketingSpendProvisional = Boolean(input.marketingSpendProvisional);
  const marketingQualityReasons = Array.isArray(input.marketingQualityReasons)
    ? input.marketingQualityReasons.map(text).filter(Boolean)
    : [];
  const qualityReasons = [...new Set(marketingQualityReasons)];
  const addQualityReason = reason => {
    if (!qualityReasons.includes(reason)) qualityReasons.push(reason);
  };
  if (costCoverage != null && costCoverage < 0.95) {
    addQualityReason(`Only ${(costCoverage * 100).toFixed(1)}% of product net sales activity has Shopify cost recorded.`);
  }
  if (adjustmentRate > 0.01) {
    addQualityReason(`${money(residualSaleAdjustments).toLocaleString("en-GB", { style: "currency", currency: "GBP" })} of unmatched Shopify sale adjustments has no product GP or COGS.`);
  }
  const refundSettlementNote = pendingRefundAmount > 0
    ? `${money(pendingRefundAmount).toLocaleString("en-GB", { style: "currency", currency: "GBP" })} across ${pendingRefundCount.toLocaleString("en-GB")} refund transaction${pendingRefundCount === 1 ? "" : "s"} is awaiting payment settlement. These refunds are already represented in Shopify reversal activity; do not add the Pending settlement amount to Product reversals.${matchedPendingRefundAdjustment > 0 ? ` Accrued sales also excludes ${money(matchedPendingRefundAdjustment).toLocaleString("en-GB", { style: "currency", currency: "GBP" })} of Shopify's matching reporting hold.` : ""}`
    : "";
  ...
  const sales = finance.salesFinancials({
    ...
    costOfGoods: cogs,
    grossProfit: input.productGrossProfit ?? input.grossProfit,
    costedNetSalesExVat: input.grossMarginRevenue,
    salesIncludeVat: false
  });
  const productGrossProfit = money(sales.grossProfit);
  const grossProfit = money(input.grossProfit ?? productGrossProfit);
  const operatingRevenue = money(input.operatingRevenue ?? (accruedNetRevenue + shippingRevenue));
  ...
  return {
    ...
    productGrossProfit,
    grossProfit,
    grossMarginRevenue: money(sales.grossMarginRevenueExVat),
    grossMargin: sales.grossMargin ?? 0,
    aovBasisRevenue,
    aov: orders > 0 ? aovBasisRevenue / orders : 0,
    itemsPerOrder: orders > 0 ? units / orders : 0,
    productReversalRevenue,
    productReversalRate,
    productReversalNetRevenue,
    shippingReversalRevenue,
    saleAdjustments,
    matchedPendingRefundAdjustment,
    residualSaleAdjustments,
    accruedSalesRevenue,
    accruedNetRevenue,
    refundAuditAvailable,
    refundAuditStatus,
    pendingRefundAmount,
    ...
    refundRecordCount,
    refundSettlementNote,
    newShippingRevenue,
    otherSalesRevenue,
    excludedRevenueActivity,
    revenueClassificationNote,
    costedNetSales,
    uncostedNetSales,
    costCoverage,
    adjustmentRate,
    profitProvisional: qualityReasons.length > 0,
    profitQualityReasons: qualityReasons,
    marketingSpendProvisional,
    marketingQualityReasons,
    marketingDataQuality: input.marketingDataQuality && typeof input.marketingDataQuality === "object"
      ? input.marketingDataQuality
      : null,
    ...
    operatingCosts,
    operatingProfit,
    operatingMargin: operatingRevenue > 0 ? operatingProfit / operatingRevenue : 0,
...
function shopifyQlSalesActualsFromRow(row = {}, rangeInput = {}) {
  return normalizeActuals({
    ...
    returns: Math.abs(reportNumber(row, "sales_reversals") || reportNumber(row, "returns")),
...
function shopifyQlTotal(rows = [], name) {
  const totalName = `${name}__totals`;
  const totalRow = rows.find(row => row && row[totalName] != null);
  if (totalRow) return reportNumber(totalRow, totalName);
  if (rows.length === 1) return reportNumber(rows[0], name);
  return rows.reduce((sum, row) => sum + reportNumber(row, name), 0);
}

function shopifyQlRowMatches(row = {}, options = {}) {
  if (options.orderType && text(row.order_or_sales_reversal).toLowerCase() !== options.orderType) return false;
  if (options.lineType && text(row.line_type).toLowerCase() !== options.lineType) return false;
  if (options.adjustment != null && Boolean(row.is_sale_adjustment) !== options.adjustment) return false;
  return true;
}

function shopifyQlRowSum(rows = [], name, options = {}) {
  return rows
    .filter(row => shopifyQlRowMatches(row, options))
    .reduce((sum, row) => sum + reportNumber(row, name), 0);
}

function shopifyQlSalesActualsFromRows(rowsInput = [], rangeInput = {}) {
  const rows = Array.isArray(rowsInput) ? rowsInput : [];
  if (!rows.length) return shopifyQlSalesActualsFromRow({}, rangeInput);
  const isGrouped = rows.some(row => row && (
    row.order_or_sales_reversal != null
    || row.line_type != null
    || row.is_sale_adjustment != null
  ));
  if (!isGrouped) return shopifyQlSalesActualsFromRow(rows[0], rangeInput);

  const orderProduct = { orderType: "order", lineType: "product", adjustment: false };
  const orderShipping = { orderType: "order", lineType: "shipping", adjustment: false };
  const reversalProduct = { orderType: "reversal", lineType: "product", adjustment: false };
  const reversalShipping = { orderType: "reversal", lineType: "shipping", adjustment: false };
  const saleAdjustments = money(shopifyQlRowSum(rows, "total_sales", { adjustment: true }));
  const demandRevenue = money(shopifyQlRowSum(rows, "total_sales", orderProduct));
  const newShippingRevenue = money(shopifyQlRowSum(rows, "total_sales", orderShipping));
  const productReversalRevenue = money(shopifyQlRowSum(rows, "total_sales", reversalProduct));
  const productReversalNetRevenue = money(shopifyQlRowSum(rows, "net_sales", reversalProduct));
  const shippingReversalRevenue = money(shopifyQlRowSum(rows, "total_sales", reversalShipping));
  const despatchRevenue = money(shopifyQlTotal(rows, "total_sales"));
  const knownBridge = demandRevenue + newShippingRevenue + productReversalRevenue + shippingReversalRevenue + saleAdjustments;
  const otherSalesRevenue = money(despatchRevenue - knownBridge);

  return normalizeActuals({
    range: validateRange(rangeInput),
    netRevenue: shopifyQlTotal(rows, "net_sales"),
    grossRevenue: shopifyQlTotal(rows, "gross_sales"),
    demandRevenue,
    aovBasisRevenue: demandRevenue,
    despatchRevenue,
    shippingRevenue: shopifyQlTotal(rows, "shipping_charges"),
    tax: shopifyQlTotal(rows, "taxes"),
    discounts: Math.abs(shopifyQlTotal(rows, "discounts")),
    returns: Math.abs(productReversalRevenue),
    productReversalRevenue,
    productReversalNetRevenue,
    shippingReversalRevenue,
    saleAdjustments,
    newShippingRevenue,
    otherSalesRevenue,
    returnFees: shopifyQlTotal(rows, "return_fees"),
    orders: shopifyQlTotal(rows, "orders"),
    units: Math.max(0, shopifyQlTotal(rows, "quantity_ordered")),
    grossUnits: Math.max(0, shopifyQlTotal(rows, "quantity_ordered")),
    returnedUnits: Math.abs(shopifyQlTotal(rows, "reversed_quantity")),
    cogs: shopifyQlTotal(rows, "cost_of_goods_sold"),
    grossProfit: shopifyQlTotal(rows, "gross_profit"),
    costedNetSales: shopifyQlTotal(rows, "net_sales_with_cost_recorded"),
    uncostedNetSales: shopifyQlTotal(rows, "net_sales_without_cost_recorded")
  });
}
...
function buildPnl(actualsInput, costRules = [], marketingEntries = []) {
  ...
  // Shopify reports product gross profit. Net shipping revenue is a separate
  // ex-VAT contribution because courier/postage costs are already charged below.
  const productGrossProfit = money(base.productGrossProfit);
  const grossProfit = money(productGrossProfit + base.shippingRevenue);
  ...
  const grossProfitPerOrder = base.orders > 0 ? money(grossProfit / base.orders) : 0;
  const operatingProfitPerOrder = base.orders > 0 ? money(operatingProfit / base.orders) : 0;
  ...
  const reconciliation = {
    revenueDifference: money(totals.operatingRevenue - (totals.accruedNetRevenue + totals.shippingRevenue)),
    grossProfitDifference: money(totals.grossProfit - (totals.productGrossProfit + totals.shippingRevenue)),
    netProfitDifference: money(totals.operatingProfit - (totals.grossProfit - operatingCosts))
  };
  reconciliation.passed = Object.values(reconciliation).every(value => typeof value === "boolean" || Math.abs(number(value)) < 0.01);
  return {
    ...totals,
    costRuleTotal,
    fixedCostTotal,
    fixedCostPerOrder,
    fixedCostImpact: totals.operatingRevenue > 0 ? fixedCostTotal / totals.operatingRevenue : 0,
    contributionProfit,
    contributionMargin: totals.operatingRevenue > 0 ? contributionProfit / totals.operatingRevenue : 0,
    grossProfitPerOrder,
    operatingProfitPerOrder,
    variableCostTotal,
    ...
    reconciliation,
    warnings: reconciliation.passed
      ? totals.warnings
      : [...totals.warnings, "P&L reconciliation failed: revenue, gross profit or net profit does not match its component bridge."],
    ...
    warnings: [
      ...base.warnings,
      ...base.profitQualityReasons.map(message => `Profit is provisional: ${message}`),
...
function scenarioDemandBasis(actuals = {}) {
  return number(actuals.demandRevenue || actuals.aovBasisRevenue || actuals.despatchRevenue || actuals.netRevenue);
}
...
function scenarioBaseDrivers(actuals) {
  ...
  return {
    targetDailyDemand: base.range.days > 0 ? scenarioDemandBasis(base) / base.range.days : 0,
    aovDelta: 0,
    itemsPerOrder: base.itemsPerOrder || 1,
    grossMarginOverride: "",
    marketingSpend: null,
    marketingDrivesSales: false,
    marketingReturn: marketingSpend > 0 ? base.demandRevenue / marketingSpend : 0
  };
}
...
function scenarioActuals(actualsInput, driversInput = {}) {
  ...
  const baseDemand = scenarioDemandBasis(actuals);
  ...
  const requestedDailyDemand = driversInput.targetDailyDemand ?? driversInput.targetDailySales ?? drivers.targetDailyDemand;
  const targetDailyDemand = Math.max(0, number(requestedDailyDemand, baseDemand / Math.max(actuals.range.days, 1)));
  const targetDemandRevenue = money(targetDailyDemand * actuals.range.days);
  ...
  const demandRevenue = drivers.marketingDrivesSales
    ? money(Math.max(0, drivers.targetDailySalesIncludesMarketing ? targetDemandRevenue : targetDemandRevenue + marketingRevenueDelta))
    : targetDemandRevenue;
  const scale = baseDemand > 0 ? demandRevenue / baseDemand : 1;
  const scaled = value => money(number(value) * scale);
  const netRevenue = scaled(actuals.netRevenue);
  const accruedNetRevenue = scaled(actuals.accruedNetRevenue);
  const despatchRevenue = scaled(actuals.despatchRevenue);
  const aov = Math.max(0.01, actuals.aov + number(drivers.aovDelta));
  const aovBasisRevenue = demandRevenue;
  const orders = aovBasisRevenue > 0 ? aovBasisRevenue / aov : 0;
  const itemsPerOrder = Math.max(0, number(drivers.itemsPerOrder, actuals.itemsPerOrder || 1));
  const units = orders * itemsPerOrder;
  const hasGrossMarginOverride = drivers.grossMarginOverride !== "" && drivers.grossMarginOverride != null;
  const grossMargin = hasGrossMarginOverride ? rate(drivers.grossMarginOverride) : actuals.grossMargin;
  const costedNetSales = scaled(actuals.costedNetSales || actuals.grossMarginRevenue);
  const uncostedNetSales = scaled(actuals.uncostedNetSales);
  const productGrossProfit = hasGrossMarginOverride
    ? money(costedNetSales * grossMargin)
    : scaled(actuals.productGrossProfit);
  const cogs = hasGrossMarginOverride
    ? money(costedNetSales - productGrossProfit)
    : scaled(actuals.cogs);
  return normalizeActuals({
    range: actuals.range,
    netRevenue,
    accruedNetRevenue,
    grossRevenue: scaled(actuals.grossRevenue),
    demandRevenue,
    aovBasisRevenue,
    despatchRevenue,
    discounts: scaled(actuals.discounts),
    returns: scaled(actuals.returns),
    productReversalRevenue: scaled(actuals.productReversalRevenue),
    productReversalNetRevenue: scaled(actuals.productReversalNetRevenue),
    shippingReversalRevenue: scaled(actuals.shippingReversalRevenue),
    saleAdjustments: scaled(actuals.saleAdjustments),
    matchedPendingRefundAdjustment: scaled(actuals.matchedPendingRefundAdjustment),
    residualSaleAdjustments: scaled(actuals.residualSaleAdjustments),
    accruedSalesRevenue: scaled(actuals.accruedSalesRevenue),
    shippingRevenue: scaled(actuals.shippingRevenue),
    newShippingRevenue: scaled(actuals.newShippingRevenue),
    otherSalesRevenue: scaled(actuals.otherSalesRevenue),
    tax: scaled(actuals.tax),
    returnFees: scaled(actuals.returnFees),
    orders,
    units,
    returnedUnits: scaled(actuals.returnedUnits),
    cogs,
    productGrossProfit,
    grossProfit: productGrossProfit,
    grossMarginRevenue: costedNetSales,
    costedNetSales,
    uncostedNetSales,
...
function leveragePoint(pnl, days) {
  return {
    range: pnl.range,
    dailyDemand: days > 0 ? money(pnl.demandRevenue / days) : 0,
    dailyDespatch: days > 0 ? money(pnl.demandRevenue / days) : 0,
    demandRevenue: pnl.demandRevenue,
...
function operatingLeverage(actualsInput, costRules = [], marketingEntries = [], driversInput = {}) {
  ...
  const actualDaily = scenarioDemandBasis(actualPnl) / days;
  const selectedDaily = Math.max(0, number(driversInput.targetDailyDemand ?? driversInput.targetDailySales, actualDaily));
  ...
  const buildAtDaily = dailyDemand => buildPnl(
    scenarioActuals(actualPnl, {
      ...leverageDrivers,
      targetDailyDemand: Math.max(0, number(dailyDemand))
    }),
    costRules,
  ...
  const lowFixedDrag = number(selected.fixedCostTotal) <= 0
    ? buildAtDaily(0)
    : findDailyThreshold(buildAtDaily, point => number(point.operatingRevenue) > 0 && number(point.fixedCostImpact) <= fixedDragThreshold, selectedDaily);
  ...
  if (breakEven) dailyValues.add(breakEven.demandRevenue / days);
  if (lowFixedDrag) dailyValues.add(lowFixedDrag.demandRevenue / days);
...
function buildScenario(actualsInput, costRules = [], marketingEntries = [], driversInput = {}) {
  ...
  const keys = ["despatchRevenue", "accruedSalesRevenue", "accruedNetRevenue", "operatingRevenue", "demandRevenue", "grossRevenue", "netRevenue", "discounts", "returns", "productReversalRevenue", "shippingRevenue", "newShippingRevenue", "orders", "units", "aov", "cogs", "productGrossProfit", "grossProfit", "grossProfitPerOrder", "grossMargin", "fixedCostTotal", "fixedCostPerOrder", "fixedCostImpact", "contributionProfit", "contributionMargin", "variableCostTotal", "variableCostPerOrder", "orderVariableCostTotal", "orderVariableCostPerOrder", "revenueVariableCostTotal", "marketingSpend", "operatingCosts", "operatingProfit", "operatingProfitPerOrder", "operatingMargin", "profitPerDay"];
  ...
  const leverageDrivers = {
    ...driversInput,
    targetDailyDemand: scenario.range.days > 0 ? scenario.demandRevenue / scenario.range.days : 0,
  ...
  return {
    ...
    drivers: {
      targetDailyDemand: money(number(driversInput.targetDailyDemand ?? driversInput.targetDailySales, scenarioDemandBasis(actualPnl) / Math.max(actualPnl.range.days, 1))),
      targetDailySales: money(number(driversInput.targetDailyDemand ?? driversInput.targetDailySales, scenarioDemandBasis(actualPnl) / Math.max(actualPnl.range.days, 1))),
...
function sensitivityTables(actualsInput, costRules = [], marketingEntries = [], driversInput = {}) {
  const actualPnl = buildPnl(actualsInput, costRules, marketingEntries);
  const baseDaily = actualPnl.range.days > 0 ? scenarioDemandBasis(actualPnl) / actualPnl.range.days : 0;
  const baseMarketing = actualPnl.marketingSpend;
  const requestedDaily = driversInput.targetDailyDemand ?? driversInput.targetDailySales;
  const selectedDaily = requestedDaily == null || requestedDaily === "" ? baseDaily : number(requestedDaily, baseDaily);
  ...
  return {
    dailySales: dailySalesSteps.map(value => buildScenario(actualPnl, costRules, marketingEntries, { ...driversInput, targetDailyDemand: value }).scenario),
