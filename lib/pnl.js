...
function calculateCostRule(ruleInput, baseInput, rangeInput) {
  ...
  if (rule.costType === "fixed_monthly") {
    ...
    amountApplied = rule.rate * overlapped.demandRevenue;
    revenueDrivenAmount = amountApplied;
    formula = `${money(rule.rate * 100)}% of order demand`;
  } else if (rule.costType === "percent_revenue_plus_per_order") {
    revenueDrivenAmount = rule.rate * overlapped.demandRevenue;
    orderDrivenAmount = rule.amount * overlapped.orders;
    amountApplied = revenueDrivenAmount + orderDrivenAmount;
    formula = `${money(rule.rate * 100)}% of order demand + ${money(rule.amount)} per order`;
  }
  return {
    ...rule,
...
function scenarioActuals(actualsInput, driversInput = {}) {
  ...
  const returnedUnitsRatio = baseDespatch > 0 ? actuals.returnedUnits / baseDespatch : 0;
  ...
  const aovBasisRatio = baseDespatch > 0 ? actuals.aovBasisRevenue / baseDespatch : 1;
  const netRevenue = money(despatchRevenue * netRatio);
  const aov = Math.max(0.01, actuals.aov + number(drivers.aovDelta));
  const aovBasisRevenue = money(despatchRevenue * aovBasisRatio);
  ...
  const productGrossProfit = hasGrossMarginOverride
    ? money(netRevenue * grossMargin)
    : money(actuals.productGrossProfit * (baseDespatch > 0 ? despatchRevenue / baseDespatch : 1));
  const cogs = money(netRevenue - productGrossProfit);
  return normalizeActuals({
    range: actuals.range,
    netRevenue,
    ...
    orders,
    units,
    returnedUnits: money(despatchRevenue * returnedUnitsRatio),
    cogs,
    productGrossProfit,
...
function buildScenario(actualsInput, costRules = [], marketingEntries = [], driversInput = {}) {
  ...
  const keys = ["despatchRevenue", "accruedSalesRevenue", "accruedNetRevenue", "demandRevenue", "grossRevenue", "netRevenue", "discounts", "returns", "shippingRevenue", "orders", "units", "aov", "cogs", "grossProfit", "grossProfitPerOrder", "grossMargin", "fixedCostTotal", "fixedCostPerOrder", "fixedCostImpact", "contributionProfit", "contributionMargin", "variableCostTotal", "variableCostPerOrder", "orderVariableCostTotal", "orderVariableCostPerOrder", "revenueVariableCostTotal", "marketingSpend", "operatingCosts", "operatingProfit", "operatingProfitPerOrder", "operatingMargin", "profitPerDay"];
