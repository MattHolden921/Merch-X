...
const DEFAULT_CHANNELS = {
  ...
  Meta: {
    ...
    accountFilterField: "account_name",
    accountIdFilterField: "account_id",
    revenueFields: ["action_values_offsite_conversion_fb_pixel_purchase"],
    revenueWeight: 0.5
  }
};
...
function spendDateSyncState(spendDate, syncedAt, finalizationDelayHours = 6) {
  const date = dateOnly(spendDate);
  const synced = syncTimestamp(syncedAt);
  if (!date || !synced) return "missing";
  const finalAfter = new Date(`${date}T00:00:00.000Z`);
  finalAfter.setUTCDate(finalAfter.getUTCDate() + 1);
  finalAfter.setUTCHours(finalAfter.getUTCHours() + Math.max(0, number(finalizationDelayHours, 6)));
  if (synced.getTime() < finalAfter.getTime()) return "partial";
  return "final";
}

function isFinalizedSpendDate(spendDate, syncedAt, finalizationDelayHours = 6) {
  return spendDateSyncState(spendDate, syncedAt, finalizationDelayHours) === "final";
}

function successfulCoverage(range = {}, runs = [], options = {}) {
  const requiredDates = datesInRange(range.startDate, range.endDate);
  const latestByDate = new Map(requiredDates.map(date => [date, ""]));
  for (const run of runs) {
    const runStart = dateOnly(run.start_date ?? run.startDate);
    const runEnd = dateOnly(run.end_date ?? run.endDate);
    const syncedAt = text(run.synced_at ?? run.syncedAt);
    if (!runStart || !runEnd || !syncedAt) continue;
    const startDate = runStart > range.startDate ? runStart : range.startDate;
    const endDate = runEnd < range.endDate ? runEnd : range.endDate;
    for (const date of datesInRange(startDate, endDate)) {
      if (!latestByDate.has(date)) continue;
      const existing = syncTimestamp(latestByDate.get(date));
      const candidate = syncTimestamp(syncedAt);
      if (candidate && (!existing || candidate.getTime() > existing.getTime())) latestByDate.set(date, syncedAt);
    }
  }
  const missingDates = [...latestByDate.entries()].filter(([, syncedAt]) => !syncedAt).map(([date]) => date);
  const states = [...latestByDate.entries()].map(([date, syncedAt]) => [
    date,
    spendDateSyncState(date, syncedAt, options.finalizationDelayHours)
  ]);
  const partialDates = states
    .filter(([, state]) => state === "partial")
    .map(([date]) => date);
  const syncedDates = [...latestByDate.values()].filter(Boolean).sort((a, b) => {
    return (syncTimestamp(a)?.getTime() || 0) - (syncTimestamp(b)?.getTime() || 0);
  });
  return {
    covered: requiredDates.length > 0 && missingDates.length === 0,
    complete: requiredDates.length > 0 && missingDates.length === 0 && partialDates.length === 0,
    synced_at: syncedDates[0] || "",
    latest_synced_at: syncedDates[syncedDates.length - 1] || "",
    missingDates,
    partialDates,
    dates: Object.fromEntries(latestByDate)
  };
}
...
function configuredChannels(env = process.env) {
  return Object.fromEntries(Object.entries(DEFAULT_CHANNELS).map(([name, defaults]) => {
    ...
    const revenueFields = csvList(env[`${prefix}_REVENUE_FIELDS`] || "").length
      ? csvList(env[`${prefix}_REVENUE_FIELDS`])
      : defaults.revenueFields;
    const revenueWeight = number(env[`${prefix}_REVENUE_WEIGHT`], defaults.revenueWeight);
    return [name, {
      ...defaults,
...
function channelFields(channel = {}, options = {}) {
  const required = [channel.accountIdFilterField || "account_id", channel.accountFilterField || "account_name"];
  if (options.includeRevenue !== false) required.push(...(channel.revenueFields || []));
  return ensureFields(channel.fields || "date,campaign,spend", required);
}
...
function attributedRevenueValue(row, fields = []) {
...
function normalizeRows(response, options = {}) {
  ...
  const revenueFields = Array.isArray(options.revenueFields) ? options.revenueFields : [];
  const revenueWeight = number(options.revenueWeight, 1);
  return rowsFromResponse(response).map((row, index) => {
    ...
    const attributedRevenue = attributedRevenueValue(row, revenueFields);
    ...
    return {
      ...
      campaignId,
      campaignName,
      attributedRevenue: attributedRevenue.value,
      attributedRevenueField: attributedRevenue.field,
      attributionWeight: revenueWeight,
      sourceRowKey,
      raw: row
    };
...
function aggregateDaily(rows = []) {
  ...
  for (const row of rows) {
    ...
    const current = groups.get(key) || {
      ...
      attributedRevenue: 0,
      attributionWeight: row.attributionWeight == null ? 1 : number(row.attributionWeight, 1),
      rowCount: 0
    };
    current.amount += number(row.amount);
    current.attributedRevenue += number(row.attributedRevenue);
    current.rowCount += 1;
