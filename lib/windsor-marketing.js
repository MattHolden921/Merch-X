...
function accountFilterForChannel(channel = {}) {
  const idField = text(channel.accountIdFilterField) || "account_id";
  const nameField = text(channel.accountFilterField) || "account_name";
  const idConditions = (channel.accountIds || []).map(id => [idField, "eq", text(id)]);
...
function buildWindsorUrl({ apiKey, connector, fields, startDate, endDate, refreshSince = "", refreshInterval = "", filter = null }) {
...
function rowMatchesAllowedAccount(row, channel = {}) {
  ...
  if ((channel.accountIds || []).some(id => text(id) && text(id) === accountId)) return true;
