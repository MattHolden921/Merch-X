...
const DEFAULT_MARKDOWN_STEPS = [10, 20, 30, 40, 50, 60];
...
function normalizeSaleCollectionConfig(input = {}) {
  const childCollectionByType = {};
  if (input.childCollectionByType && typeof input.childCollectionByType === "object") {
    for (const [key, value] of Object.entries(input.childCollectionByType)) {
      const productType = text(key);
      const collectionId = text(value);
      if (productType && collectionId) childCollectionByType[productType] = collectionId;
    }
  }
  return {
    rootSaleCollectionId: text(input.rootSaleCollectionId),
    childCollectionByType
  };
}

function mergeSaleCollectionConfig(current = {}, input = {}, scopedProductTypes = []) {
  const existing = normalizeSaleCollectionConfig(current);
  const incoming = normalizeSaleCollectionConfig(input);
  const childCollectionByType = { ...existing.childCollectionByType };
  const hasIncomingMapping = Boolean(input.childCollectionByType && typeof input.childCollectionByType === "object");
  const scope = [...new Set((Array.isArray(scopedProductTypes) ? scopedProductTypes : []).map(text).filter(Boolean))];

  if (hasIncomingMapping && scope.length) {
    for (const productType of scope) delete childCollectionByType[productType];
    for (const productType of scope) {
      if (incoming.childCollectionByType[productType]) {
        childCollectionByType[productType] = incoming.childCollectionByType[productType];
      }
    }
  } else if (hasIncomingMapping) {
    Object.assign(childCollectionByType, incoming.childCollectionByType);
  }

  return {
    rootSaleCollectionId: Object.prototype.hasOwnProperty.call(input, "rootSaleCollectionId")
      ? incoming.rootSaleCollectionId
      : existing.rootSaleCollectionId,
    childCollectionByType
  };
}
...
function effectiveDiscountPercent(original, target) {
  const originalPriceValue = number(original);
  const targetPriceValue = number(target);
  if (!(originalPriceValue > 0) || !(targetPriceValue > 0)) return 0;
  return Math.round((((originalPriceValue - targetPriceValue) / originalPriceValue) * 100) * 10) / 10;
}

function validateSaleItem(item = {}, options = {}) {
  const mode = text(options.mode || "apply").toLowerCase();
  const minGpPct = number(options.minGpPct, 0);
  const requireCostAtDiscount = number(options.requireCostAtDiscount, 40);
  const errors = [];
  const warnings = [];
  const variants = Array.isArray(item.variants) ? item.variants : [];
  const expectedStatus = mode === "remove" ? "Applied" : "Planned";
  if (text(item.status) !== expectedStatus && !(text(item.status) === "Error" && options.allowRetry)) {
    errors.push(`${mode === "remove" ? "Remove" : "Apply"} requires ${expectedStatus} rows.`);
  }
  if (!text(item.shopifyProductId)) errors.push("Shopify product ID is missing.");
  if (!variants.length) errors.push("Variant price targets are missing.");
  if (mode === "apply" && number(item.stock) <= 0) errors.push("Product has no stock available for sale.");
  if (mode === "apply" && !text(item.rootSaleCollectionId)) errors.push("Root Sale collection is missing.");
  if (mode === "apply" && !text(item.childSaleCollectionId)) warnings.push("Child Sale collection is missing.");
  for (const variant of variants) {
    const label = text(variant.sku || variant.title || variant.id || "Variant");
    const original = number(variant.saleOriginalPrice || variant.originalPrice || item.originalPrice);
    const target = number(variant.targetPrice || item.targetPrice);
    const discount = effectiveDiscountPercent(original, target);
    const gp = gpPercentFromRetail(target, variant.cost);
    if (!text(variant.id)) errors.push(`${label}: variant ID is missing.`);
    if (!(original > 0)) errors.push(`${label}: original price is missing.`);
    if (!(target > 0)) errors.push(`${label}: target price must be greater than zero.`);
    if (original > 0 && target > original) errors.push(`${label}: target price cannot exceed original price.`);
    if (mode === "apply" && discount >= requireCostAtDiscount && (variant.cost == null || variant.cost === "")) {
      errors.push(`${label}: cost is required for markdowns of ${requireCostAtDiscount}% or more.`);
    } else if (mode === "apply" && (variant.cost == null || variant.cost === "")) {
      warnings.push(`${label}: cost is missing, so target GP cannot be verified.`);
    }
    if (mode === "apply" && gp != null && gp < minGpPct) errors.push(`${label}: target GP ${gp.toFixed(1)}% is below the ${minGpPct.toFixed(1)}% floor.`);
  }
...
function deepenExistingMarkdown(step, existingMarkdown) {
  ...
  return Math.max(step, nextStep || DEFAULT_MARKDOWN_STEPS[DEFAULT_MARKDOWN_STEPS.length - 1]);
}
...
function markdownLearningStep(product = {}, baseStep, outcomes = []) {
  ...
  if (similar.length < 4) return step;
  ...
  if (failedAtOrBelow.length >= 2 && deeperWorked.length >= 2) {
  ...
  if (lighterWorked.length >= 3 && !failedAtOrBelow.length) {
...
function recommendMarkdown(product = {}, options = {}) {
  ...
  if (signals.stock <= 0) warnings.push("Product has no stock available for sale.");
  return {
...
function variantSaleTargets(product = {}, discountPercent, options = {}) {
  ...
  return variants.map(variant => {
    ...
    if (cost == null) warnings.push("Variant cost is missing.");
...
function nextMarkdownStep(currentDiscount) {
  ...
  return DEFAULT_MARKDOWN_STEPS.find(candidate => candidate > current) || 60;
}

function manualMarkdownActionRecommendation(outcome = {}, item = {}, targetDiscountPercent = 60, options = {}) {
  const targetDiscount = number(targetDiscountPercent);
  if (!DEFAULT_MARKDOWN_STEPS.includes(targetDiscount)) {
    throw new Error(`Choose a markdown target from ${DEFAULT_MARKDOWN_STEPS.join("%, ")}%.`);
  }
  ...
  if (!(originalPriceValue > 0)) throw new Error("An original retail price is required to force a further markdown.");
  if (targetDiscount <= currentDiscount) throw new Error(`Choose a target deeper than the current ${currentDiscount}% markdown.`);
  return {
    actionType: "deepen",
    label: "Deepen markdown",
    priority: targetDiscount >= 50 ? "High" : "Medium",
    currentDiscountPercent: currentDiscount,
    recommendedDiscountPercent: targetDiscount,
    currentPrice: money(currentPriceValue),
    recommendedTargetPrice: targetPriceForDiscount(originalPriceValue, targetDiscount, options.roundingRule || "nearest-pound"),
    originalPrice: money(originalPriceValue),
    reason: `Commercial override to support a ${targetDiscount}% off sale message; evidence outcome remains ${text(outcome.outcome || "watch").toLowerCase()}.`,
    data: { manualOverride: true, evidenceOutcome: text(outcome.outcome || "watch").toLowerCase(), targetDiscountPercent: targetDiscount }
  };
}
