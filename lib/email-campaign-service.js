...
function createEmailCampaignService(deps) {
  ...
  function productGrid(campaign) { const rows = []; for (let index = 0; index < campaign.products.length; index += 2) { const cells = campaign.products.slice(index, index + 2).map(item => `<td width="50%" valign="top" style="padding:8px"><a href="${html(item.trackedUrl)}" style="color:#172033;text-decoration:none"><img src="${html(item.imageUrl)}" alt="${html(item.imageAlt || item.title)}" width="280" style="display:block;width:100%;height:auto"><h2 style="font-size:16px;line-height:1.35;margin:12px 0 5px">${html(item.title)}</h2><p style="font-size:15px;margin:0 0 14px">£${Number(item.price || 0).toFixed(2)}</p><span style="display:inline-block;background:#172033;color:#fff;padding:10px 16px;font-size:13px;font-weight:bold">SHOP NOW</span></a></td>`).join(""); rows.push(`<table role="presentation" width="100%"><tr>${cells}</tr></table>`); } return rows.join(""); }
