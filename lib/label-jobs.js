...
function supplierGuideGroupKey(line) {
  return [
    normalized(line.buyingCode || line.supplierSku),
    normalized(line.style || line.description)
  ].join("|");
}
...
function normalizeDoubleBarcodeSnapshot(snapshot = {}) {
  const rows = (snapshot.rows || []).map(row => {
    ...
    return {
      ...row,
      supplierGuideGroupKey: supplierGuideGroupKey(row),
...
function buildLabelJobSnapshot({ order, batchLines = [], batches = [], scopeType = "order", batchId = "", selectedLineIndexes = [], selectionRequired = false, sparePerSku = 0, labelTemplate = "60 x 40 mm swing-ticket barcode", placementInstructions = "" } = {}) {
  ...
  for (const item of scoped) {
    ...
    row.supplierGuideGroupKey = supplierGuideGroupKey(row);
...
module.exports = { buildLabelJobSnapshot, matchKey, normalizeDoubleBarcodeSnapshot, scopeQuantities, supplierGuideGroupKey };
