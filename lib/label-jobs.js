...
function scopeQuantities(order, batchLines = [], scopeType = "order", batchId = "") {
...
function buildLabelJobSnapshot({ order, batchLines = [], batches = [], scopeType = "order", batchId = "", sparePerSku = 0, labelTemplate = "60 x 40 mm swing-ticket barcode", placementInstructions = "" } = {}) {
  const errors = [];
  const warnings = [];
  const scoped = scopeQuantities(order, batchLines, scopeType, batchId);
  const spare = Math.max(0, Math.min(100, Math.floor(Number(sparePerSku || 0))));
  if (!scoped.length) errors.push("The selected scope has no units to label.");
