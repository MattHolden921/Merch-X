...
- ShopifyQL sales reports are the source for P&L actuals. Date ranges are inclusive and capped at 366 days. The page can load one second actualised period through the same P&L endpoint and compare its full statement with the primary period. The comparison defaults to the immediately preceding equal-length range and supports custom dates. Its result mirrors the scenario-result hierarchy: comparison KPI tiles show the primary value and comparison-minus-primary delta underneath, followed by a detailed actuals table whose two value columns are labelled with their date ranges and whose final column shows Delta. Daily-normalised profit tiles keep unequal-length ranges interpretable. Actual comparison clears when new primary actuals load and is not persisted to SQLite.
- Primary P&L sales views are Despatch and Demand. Despatch maps to ShopifyQL `total_sales`. Demand is derived as product sales inc VAT, after discounts, before returns: `(gross_sales - absolute_discounts) * (1 + effective VAT rate)`. ShopifyQL `gross_sales` remains visible only as gross sales ex VAT before discounts and returns. Discounts, returns, shipping, net sales, and taxes are shown as separate sales-bridge boxes because they materially explain the movement from Demand to Despatch. Net sales remains the ex-VAT product revenue used for net-profit calculations. The top summary shows Gross profit as a pound value with Gross profit % as its supporting value. Shopify only reports GP and COGS for sales with product cost recorded, so actual Gross profit % uses the matching costed net sales basis: `gross profit / (gross profit + COGS)`. This is equivalent to the merchandising convention `((retail ex VAT) - cost) / retail ex VAT` without mixing Shopify's costed profit subset with total net sales. Net profit is the pound operating-profit value after COGS, marketing, variable costs, and fixed overheads; Net profit % remains `net profit / total net sales ex VAT`.
